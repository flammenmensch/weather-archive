...
var UPDATE_START_DATE = exports.UPDATE_START_DATE = 'app/updateStartDate';
var UPDATE_END_DATE = exports.UPDATE_END_DATE = 'app/updateEndDate';
var UPDATE_DATA_TYPE = exports.UPDATE_DATA_TYPE = 'app/updateDataSet';
...
var DATABASE_NAME = 'weather_archive';
...
var createStore = function createStore(db, name) {
  var options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  var indexes = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];

  var store = db.createObjectStore(name, options);
  indexes.forEach(function (idx) {
    return store.createIndex(idx.name, idx.key, idx.options);
  });
  return store;
};

var hasStore = function hasStore(db, name) {
  return db.objectStoreNames.contains(name);
};

var getStore = function getStore(name, mode) {
...
var queryStore = function queryStore(store) {
  return new Promise(function (resolve, reject) {
...
var countRecords = function countRecords(store) {
...
var save = function save(name, json) {
...
var exists = function exists(name) {
  return getStore(name, 'readonly').then(countRecords).then(function (result) {
    return result > 0;
  });
};

var loadFromDb = function loadFromDb(name) {
  return getStore(name, 'readonly').then(queryStore);
};
...
var _dbService = __webpack_require__(126);
...
var makeRenderer = function makeRenderer(Component) {
  return function () {
    var state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    return document.querySelector('#app').innerHTML = Component(state);
  };
};

var init = function init() {
  var update = makeRenderer(_App2.default);
  var initialDataSet = TEMPERATURE;

  (0, _dbService.load)(initialDataSet).then(function (data) {
    var minDate = data[0].t;
    var maxDate = data[data.length - 1].t;
    var initialState = {
      buttons: [{ label: 'Temperature', value: TEMPERATURE }, { label: 'Precipitation', value: PRECIPITATION }],
    ...
    var store = (0, _createStore2.default)(_reducers2.default, update, initialState);
...
exports.default = function (reducer, onStoreUpdate) {
  ...
  var state = _extends({}, initialState, { dispatch: dispatch });
...
var _actionTypes = __webpack_require__(125);
...
exports.default = function () {
  ...
  switch (action.type) {
    case T.UPDATE_START_DATE:
      return _extends({}, state, { selectedStartDate: action.payload });
    case T.UPDATE_END_DATE:
      return _extends({}, state, { selectedEndDate: action.payload });
...
var _chartService = __webpack_require__(334);

var chartService = _interopRequireWildcard(_chartService);

var _dbService = __webpack_require__(126);

var _Chart = __webpack_require__(335);

var _Chart2 = _interopRequireDefault(_Chart);

var _DatePicker = __webpack_require__(336);

var _DatePicker2 = _interopRequireDefault(_DatePicker);

var _RadioButtonGroup = __webpack_require__(337);
...
var createPoints = chartService.getPoints(CHART_WIDTH, CHART_HEIGHT);

exports.default = function (props) {
  var buttons = new _RadioButtonGroup2.default({
    items: props.buttons,
    name: props.buttonGroupName,
    selectedValue: props.selectedDataSet,
    onChange: function onChange(event) {
      var dataSet = event.target.value;
      (0, _dbService.load)(dataSet).then(function (data) {
  ...
  var startDate = new _DatePicker2.default({
    min: props.minDate,
    max: props.maxDate,
  ...
  var endDate = new _DatePicker2.default({
    min: props.minDate,
    max: props.maxDate,
...
var findIndexPredicate = function findIndexPredicate(value) {
...
var linearInterpolate = function linearInterpolate(before, after, atPoint) {
  return before.v + (after.v - before.v) * atPoint;
};

var interpolate = function interpolate(data, count) {
  var newData = [data[0]];
  var springFactor = (data.length - 1) / (count - 1);
...
var getPoints = exports.getPoints = function getPoints(width, height) {
  return function (source, from, to) {
    var startIndex = source.findIndex(findIndexPredicate(from));
    var endIndex = source.findIndex(findIndexPredicate(to));

    var sliced = source.slice(startIndex, endIndex);

    return interpolate(sliced, width).map(function (_ref, index) {
      var v = _ref.v;
      return { x: index, y: width / 2 - v };
    });
  };
};
...
var _Component2 = __webpack_require__(89);
...
var Chart = function (_Component) {
  ...
  function Chart() {
    ...
    _this.state = { width: width, height: height, points: points };
    return _this;
  }
  ...
  _createClass(Chart, [{
    ...
    value: function render() {
      ...
      var start = this.state.points[0];
      ctx.moveTo(start.x, start.y);

      var point = void 0;
      for (var i = 1, n = this.state.points.length; i < n; i++) {
...
var DatePicker = function (_Component) {
  ...
  function DatePicker() {
    var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
        _ref$min = _ref.min,
        min = _ref$min === undefined ? undefined : _ref$min,
        _ref$max = _ref.max,
    ...
    _this.state = { min: min, max: max, selectedValue: selectedValue, onChange: onChange };
    return _this;
  }
...
var createRadioButtons = function createRadioButtons(items, name, selectedValue, handler) {
  return items.map(function (item, index) {
    return '<label for="radio-' + index + '">\n        <input\n          onchange="' + handler + '"\n          id="radio-' + index + '"\n          type="radio"\n          name="' + name + '"\n          value="' + item.value + '"\n          ' + (selectedValue === item.value ? 'checked' : '') + '\n        >\n        ' + item.label + '\n       </label>\n      ';
  }).join('');
};
...
var RadioButtonGroup = function (_Component) {
  ...
  function RadioButtonGroup() {
    ...
    _this.state = { items: items, name: name, onChange: onChange, selectedValue: selectedValue };
    return _this;
  }
